...
public class ImgExt4Doc {
	...
	{
		File f = new File(docFile);       // File f = new File("D:\\work\\test.doc"); //       
		...
		// ArrayList <PAPX> pList=msWord.getParagraphTable().getParagraphs();
		// PAPX p=pList.get(0);
		int underlinecode = 0;
		int imgcount = 0;
		...
		for (int i = 0; i < numCharacterRuns; i++) {
			...
			for(int m =startOffset;m<endOffset;m++){
				...
				if(m<endOffset&&(hashCode == 13||hashCode==7))//get line
				{
					enterCount++;
					// System.out.println("ENTER FIND........."+enterCount);
				}
				text=text + range.text();

			}
			///////////////////
			if (pTable.hasPicture(characterRun)) {//是图片则把图片位置替换为image标签，再把图片输出到指定位置
				Picture pic = pTable.extractPicture(characterRun, true);
				String fileName = pic.suggestFullFileName();
				byte[] content=pic.getContent();
				out=new FileOutputStream(new File(imgStoreDir + File.separator + fileName));//save picture
				out.write(content);
				out.flush();
				out.close();
